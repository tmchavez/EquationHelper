...
#for now, assumes all elements are size 100.
#needs to be improved to handle exponents and subscripts
def saveToText():
    print("in save to text")
    filename = entry4.get()
    fd = open(filename,"w")
    count  = 200
    numrows = (curheight/100)+100
    wstring = ''
    while(count < numrows):
        collist = getCol(count)
        print("row: "+str(count))
        print("collist size = " + str(len(collist)))
        countj = 300
        numcol = (curheight/100)+100
        while(countj < numcol):
            for elm in collist:
                if(elm.posX == countj):
                    print(elm.txt + "was added to write string")
                    wstring = wstring+elm.text
...
buttona = Button(frame,text="+",bg="green",state="normal",command=plusSize)
buttona.grid(row=12,column=0)

buttonb = Button(frame,text="-",bg="green",state="normal",command=minusSize)
